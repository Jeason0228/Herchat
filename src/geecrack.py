...
def drag_the_ball(driver, track):
    ...
    while track:
        ...
        x = random.choice([-1, 2, -2, 1])
        print(x)
        ActionChains(driver).move_by_offset(xoffset=x, yoffset=0).perform()
        track.remove(x)
    # time.sleep(0.01)
    # 模拟人往回滑动
    imitate = ActionChains(driver).move_by_offset(xoffset=-3, yoffset=0)
    # time.sleep(0.015)
    imitate.perform()
    ...
    # 放开圆球
    ActionChains(driver).pause(random.randint(6, 14) / 20).release(slider).perform()
